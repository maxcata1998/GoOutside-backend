...
export default class ActivsController{
  ...
  static async apiUpdateStar(req, res, next){
    try{
      const activId = req.body.activ_id;
      const addStar = req.body.rating/20;
      const getActiv = await ActivsDAO.getActivById(activId);
      const oldRating = getActiv.rating[0];
      const oldSum = getActiv.rating[1];
      const newRating = oldRating+addStar;
      const newSum = oldSum+1;
